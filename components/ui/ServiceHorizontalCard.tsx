...
export function ServiceHorizontalCard({
    ...
    return (
        <div className={cn(
            "group bg-white rounded-[10px] p-6 border border-slate-100 shadow-sm transition-all duration-500 flex flex-col sm:flex-row gap-4 items-start cursor-pointer relative overflow-hidden",
            "hover:bg-[#F3F9FF] hover:border-[#D0E5FF] hover:shadow-md sm:h-[210px]",
            className
        )}>
            {/* Image Container - Flip effect */}
            <div className="flex-shrink-0 [perspective:1000px] z-10">
                <div className="w-[75px] h-[75px] rounded-lg overflow-hidden shadow-sm relative transition-all duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
                    <img
                        src={imageUrl}
                        alt={title}
            ...
            <div className="flex-grow flex flex-col min-w-0 z-10">
                ...
                <div className="flex flex-wrap gap-2 mb-4 overflow-hidden">
                    {tags.map((tag) => (
                        <span
                            key={tag}
                            className="px-2 py-0.5 bg-[#eff6ff] text-[#334E6F] rounded-md text-[10px] font-bold tracking-tight whitespace-nowrap"
                        >
                            {tag}
                        </span>
                    ))}
